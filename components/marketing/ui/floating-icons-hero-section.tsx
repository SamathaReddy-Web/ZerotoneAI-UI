...
const Icon = ({
  ...
  return (
    ...
    >
      ...
      <motion.div
        className="flex items-center justify-center w-14 h-14 md:w-16 md:h-16 p-3 rounded-2xl shadow-lg bg-white/90 backdrop-blur-md border border-black/5"
        animate={{
          y: [0, -8, 0, 8, 0],
          x: [0, 6, 0, -6, 0],
          rotate: [0, 5, 0, -5, 0],
...
>(({ className, title, subtitle, ctaText, ctaHref, icons, ...props }, ref) => {
  ...
  return (
    <section
      ...
      className={cn(
        'relative w-full h-full min-h-[500px] flex items-center justify-center overflow-hidden bg-background',
        className
      )}
      {...props}
    >
      ...
      {/* Container for the foreground content (optional) */}
      {(title || subtitle || ctaText) && (
        <div className="relative z-10 text-center px-4">
          ...
          {subtitle && (
            <p className="mt-4 max-w-xl mx-auto text-base text-muted-foreground">{subtitle}</p>
          )}
          {ctaText && ctaHref && (
            <div className="mt-8">
              <a
                href={ctaHref}
                className="inline-block px-8 py-3 rounded-full bg-foreground text-background font-semibold text-sm hover:opacity-90 transition-opacity"
              >
                {ctaText}
              </a>
            </div>

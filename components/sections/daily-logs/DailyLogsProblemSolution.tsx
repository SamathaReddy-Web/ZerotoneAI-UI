...
import { X, NotebookPen, SearchX } from "lucide-react";

const PAIN_POINTS = [
  "Notebooks and spreadsheets: no centralized record of what happened daily",
  "Delay claims require historical proof, but logs are handwritten and scattered",
  "No weather data: just notes like \"rainy\" with no temperature or wind",
  "Crew information is rough estimates: not certified counts or trades",
];

export function DailyLogsProblemSolution() {
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 lg:gap-24 items-center">
          ...
          <motion.div variants={containerVariants} initial="hidden" animate={isInView ? "visible" : "hidden"} className="flex flex-col max-w-2xl">
            ...
            <motion.h2 variants={itemVariants} className="mb-6 text-balance font-display text-3xl font-bold tracking-tight text-text-primary sm:text-4xl md:text-5xl leading-tight">
              When you need history, it&apos;s scattered and lost.
            </motion.h2>

            <motion.p variants={itemVariants} className="mb-10 text-lg text-text-secondary leading-relaxed">
              Supers keep a notebook on site. Weather notes. Crew count. Work progress. One day it rains and work stops. Three months later, when it&apos;s time to file a schedule delay claim, the notebook is in a filing cabinet somewhere. You&apos;ve got no proof of what happened when.
            </motion.p>
              ...
              <div className="rounded-xl border border-border-subtle bg-surface/50 overflow-hidden flex flex-col gap-2 p-3">
                ...
                <motion.div variants={itemVariants} className="bg-white rounded-xl border border-border p-4 shadow-sm transition-colors hover:border-error-border">
                  <div className="flex items-start gap-3">
                    <NotebookPen className="h-6 w-6 text-text-muted mt-0.5" />
                    <div>
                      <span className="font-body text-[13.5px] font-semibold text-text-primary block">Super&apos;s Notepad (Oct 15)</span>
                      <span className="font-body text-[13px] text-text-secondary italic mt-1">&quot;Rained all morning. Concrete delayed. Few guys on site.&quot;</span>
                    </div>
                  </div>
                </motion.div>
                ...
                <motion.div variants={itemVariants} className="bg-white rounded-xl border border-border p-4 shadow-sm transition-colors hover:border-error-border mt-2">
                  <div className="flex items-start gap-3">
                    <SearchX className="h-6 w-6 text-error-text mt-0.5" />
                    <div>
                      <span className="font-body text-[13.5px] font-semibold text-text-primary block">Delay Claim Review (Feb 12)</span>
                      <span className="font-data text-[12px] text-error-text flex items-center gap-1 mt-1">
                        Cannot verify weather conditions or crew impacts.
                      </span>
                    </div>
                  </div>
                </motion.div>
                ...
                <motion.div variants={itemVariants} className="text-center p-3 mt-1">
                  <span className="font-data text-[11px] font-bold uppercase tracking-wider text-error-text bg-error-bg px-3 py-1.5 rounded-full">Claim Denied: Insufficient Evidence</span>
                </motion.div>

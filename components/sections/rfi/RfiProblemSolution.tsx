...
import { X, Mail, FileSpreadsheet, SearchX } from "lucide-react";

const PAIN_POINTS = [
  "RFIs sent to the wrong contact: discovered two weeks later when the work stalled",
  "No response timer: questions drift 20+ days with no one flagging the delay",
  "Owner asks for the RFI log: you spend half a day assembling it from email threads",
  "Dispute at closeout: no paper trail showing who approved the substitution",
];

export function RfiProblemSolution() {
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 lg:gap-24 items-center">
          ...
          <motion.div variants={containerVariants} initial="hidden" animate={isInView ? "visible" : "hidden"} className="flex flex-col max-w-2xl">
            ...
            <motion.h2 variants={itemVariants} className="mb-6 text-balance font-display text-3xl font-bold tracking-tight text-text-primary sm:text-4xl md:text-5xl leading-tight">
              Spreadsheets and email aren&apos;t an RFI system. They&apos;re a liability.
            </motion.h2>

            <motion.p variants={itemVariants} className="mb-10 text-lg text-text-secondary leading-relaxed">
              What goes wrong isn&apos;t the big stuff. It&apos;s the slow bleed: a question routed to the wrong person, a response that drifted two weeks past the 7-day SLA, a dispute at closeout because nobody can find who approved what.
            </motion.p>
            ...
            <div className="rounded-2xl border border-border bg-white p-2 shadow-overlay transform transition-transform hover:-translate-y-1 hover:shadow-2xl duration-500">
              <div className="rounded-xl border border-border-subtle bg-surface/50 overflow-hidden flex flex-col gap-2 p-3">
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div className="w-full">
                      <div className="flex justify-between items-center mb-1">
                        <span className="font-body text-[13px] font-semibold text-text-primary">Fwd: Re: RFI 42 - Steel Detail</span>
                        <span className="font-data text-[11px] text-text-muted">14 days ago</span>
                      </div>
                      <span className="font-body text-[12.5px] text-text-secondary line-clamp-2">
                        &quot;Hey Jim, I think this was supposed to go to the structural engineer, not me. Can you forward it to them?&quot;
                      </span>
                    </div>
                  </div>
                </motion.div>
                ...
                <motion.div variants={itemVariants} className="bg-white rounded-xl border border-border p-4 shadow-sm transition-colors hover:border-error-border mt-2">
                  <div className="flex items-start gap-3">
                    <FileSpreadsheet className="h-5 w-5 text-success mt-0.5" />
                    <div>
                      <span className="font-body text-[13.5px] font-semibold text-text-primary block">RFI_Log_Master_v9.xlsx</span>
                      <span className="font-data text-[12px] text-error-text flex items-center gap-1 mt-1">
                        <SearchX className="h-3.5 w-3.5" /> Missing approval documentation
                      </span>
                    </div>
                  </div>
                </motion.div>
                ...
                <motion.div variants={itemVariants} className="text-center p-3 mt-1">
                  <span className="font-data text-[11px] font-bold uppercase tracking-wider text-error-text bg-error-bg px-3 py-1.5 rounded-full">Costly delays hidden in inboxes</span>
                </motion.div>

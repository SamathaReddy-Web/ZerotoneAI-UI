...
import { X, SearchX, Mail, AlertTriangle } from "lucide-react";

const PAIN_POINTS = [
  "Vendor info scattered across email, spreadsheets, and old files",
  "Compliance docs (insurance, licenses) not tracked: expiration dates missed",
  "Lien waiver status is a mystery: did they send it or not?",
  "Payment setup is inconsistent: some vendors get paid late, some on time",
];

export function VendorProblemSolution() {
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 lg:gap-24 items-center">
          ...
          <motion.div variants={containerVariants} initial="hidden" animate={isInView ? "visible" : "hidden"} className="flex flex-col max-w-2xl">
            ...
            <motion.h2 variants={itemVariants} className="mb-6 text-balance font-display text-3xl font-bold tracking-tight text-text-primary sm:text-4xl md:text-5xl leading-tight">
              Insurance expires. Lien waivers go missing. Payment info is wrong.
            </motion.h2>

            <motion.p variants={itemVariants} className="mb-10 text-lg text-text-secondary leading-relaxed">
              You need a vendor&apos;s insurance cert: it&apos;s in an email from three months ago. Payment details are in a spreadsheet that someone updated, maybe. You contact a sub for a lien waiver, they say they sent it, you can&apos;t find it. At year end, accounting needs 1099 tax IDs but the vendor records are a mess.
            </motion.p>
            ...
            <div className="rounded-2xl border border-border bg-white p-2 shadow-overlay transform transition-transform hover:-translate-y-1 hover:shadow-2xl duration-500">
              <div className="rounded-xl border border-border-subtle bg-surface/50 overflow-hidden flex flex-col gap-2 p-3">
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div className="w-full">
                      <div className="flex justify-between items-center mb-1">
                        <span className="font-body text-[13px] font-semibold text-text-primary">Where is the COI?</span>
                        <span className="font-data text-[11px] text-text-muted">Tuesday</span>
                      </div>
                      <span className="font-body text-[12.5px] text-text-secondary line-clamp-2">
                        &quot;Hey, the drywall sub is on site but I can&apos;t find their insurance certificate anywhere. Are we sure they are covered? Also we need their W9 before we can cut the check this week.&quot;
                      </span>
                    </div>
                  </div>
                </motion.div>
                ...
                <motion.div variants={itemVariants} className="bg-white rounded-xl border border-border p-4 shadow-sm transition-colors hover:border-error-border mt-2">
                  <div className="flex items-start gap-3">
                    <AlertTriangle className="h-5 w-5 text-error-text mt-0.5" />
                    <div>
                      <span className="font-body text-[13.5px] font-semibold text-text-primary block">Missing Lien Waiver</span>
                      <span className="font-data text-[12px] text-error-text flex items-center gap-1 mt-1">
                        <SearchX className="h-3.5 w-3.5" /> Sub claims they mailed it. We don&apos;t have it.
                      </span>
                    </div>
                  </div>
                </motion.div>
                ...
                <motion.div variants={itemVariants} className="text-center p-3 mt-1">
                  <span className="font-data text-[11px] font-bold uppercase tracking-wider text-error-text bg-error-bg px-3 py-1.5 rounded-full">Manual compliance puts your firm at risk</span>
                </motion.div>

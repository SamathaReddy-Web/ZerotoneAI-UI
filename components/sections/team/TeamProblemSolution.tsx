...
import { X, SearchX, FileSpreadsheet, LockOpen } from "lucide-react";

const PAIN_POINTS = [
  "No role based access: everyone has the same access or wrong access",
  "No approval authority limits: anyone can approve any PO",
  "No audit trail. Who changed what? Impossible to know",
  "Compliance nightmare: auditors ask for access logs, you have nothing",
];

export function TeamProblemSolution() {
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 lg:gap-24 items-center">
          ...
          <motion.div variants={containerVariants} initial="hidden" animate={isInView ? "visible" : "hidden"} className="flex flex-col max-w-2xl">
            ...
            <motion.h2 variants={itemVariants} className="mb-6 text-balance font-display text-3xl font-bold tracking-tight text-text-primary sm:text-4xl md:text-5xl leading-tight">
              Who can do what? No one knows. Security is a guess.
            </motion.h2>

            <motion.p variants={itemVariants} className="mb-10 text-lg text-text-secondary leading-relaxed">
              New hire gets added to spreadsheet. Has access to everything. Or nothing, because no one remembered to set permissions. A sub gets full edit access to the budget. Finance can&apos;t see payables but somehow approved a $100K payment. Months later, you need to prove who touched what, when. You&apos;ve got no audit trail.
            </motion.p>
            ...
            <div className="rounded-2xl border border-border bg-white p-2 shadow-overlay transform transition-transform hover:-translate-y-1 hover:shadow-2xl duration-500">
              <div className="rounded-xl border border-border-subtle bg-surface/50 overflow-hidden flex flex-col gap-2 p-3">
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div className="w-full">
                      <div className="flex justify-between items-center mb-1">
                        <span className="font-body text-[13px] font-semibold text-text-primary">System Access Log</span>
                        <span className="font-data text-[11px] text-text-muted">Missing</span>
                      </div>
                      <span className="font-body text-[12.5px] text-text-secondary line-clamp-2">
                        &quot;Hey IT, the auditors need a list of everyone who approved a PO over $50k last quarter. Can you pull that?&quot; — &quot;No, our system doesn&apos;t track that.&quot;
                      </span>
                    </div>
                  </div>
                </motion.div>
                ...
                <motion.div variants={itemVariants} className="bg-white rounded-xl border border-border p-4 shadow-sm transition-colors hover:border-error-border mt-2">
                  <div className="flex items-start gap-3">
                    <LockOpen className="h-5 w-5 text-error-text mt-0.5" />
                    <div>
                      <span className="font-body text-[13.5px] font-semibold text-text-primary block">Permission Error</span>
                      <span className="font-data text-[12px] text-error-text flex items-center gap-1 mt-1">
                        <SearchX className="h-3.5 w-3.5" /> Junior PE accidentally deleted the master estimate.
                      </span>
                    </div>
                  </div>
                </motion.div>
                ...
                <motion.div variants={itemVariants} className="text-center p-3 mt-1">
                  <span className="font-data text-[11px] font-bold uppercase tracking-wider text-error-text bg-error-bg px-3 py-1.5 rounded-full">Poor access control is a massive liability</span>
                </motion.div>

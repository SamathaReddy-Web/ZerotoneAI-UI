...
import { X, SearchX, AlertOctagon, NotebookPen } from "lucide-react";

const PAIN_POINTS = [
  "Deliveries logged on paper, or not at all",
  "No live view of stock by warehouse or by job",
  "Duplicate orders because nobody could see what was already on hand",
  "Equipment moves between jobs with no record of where it is",
];

export function InventoryProblemSolution() {
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 lg:gap-24 items-center">
          ...
          <motion.div variants={containerVariants} initial="hidden" animate={isInView ? "visible" : "hidden"} className="flex flex-col max-w-2xl">
            ...
            <motion.h2 variants={itemVariants} className="mb-6 text-balance font-display text-3xl font-bold tracking-tight text-text-primary sm:text-4xl md:text-5xl leading-tight">
              Crew shows up. Material&apos;s not there. Nobody knew.
            </motion.h2>

            <motion.p variants={itemVariants} className="mb-10 text-lg text-text-secondary leading-relaxed">
              Materials get ordered against a PO, delivered to a yard or job site, and from that point nobody tracks what&apos;s actually on hand. A foreman assumes there&apos;s enough conduit on site; there isn&apos;t. The result: emergency orders at retail price and stalled crews.
            </motion.p>
              ...
              <div className="rounded-xl border border-border-subtle bg-surface/50 overflow-hidden flex flex-col gap-2 p-3">
                ...
                <motion.div variants={itemVariants} className="bg-white rounded-xl border border-border p-4 shadow-sm transition-colors hover:border-error-border">
                  <div className="flex items-start gap-3">
                    <NotebookPen className="h-5 w-5 text-text-muted mt-0.5" />
                    <div className="w-full">
                      <div className="flex justify-between items-center mb-1">
                        <span className="font-body text-[13px] font-semibold text-text-primary">Foreman&apos;s Log</span>
                        <span className="font-data text-[11px] text-text-muted">This morning</span>
                      </div>
                      <span className="font-body text-[12.5px] text-text-secondary line-clamp-2">
                        &quot;Hey we ran out of 4x8 drywall, I thought we got a delivery on Tuesday? I sent guys to Home Depot to buy some out of pocket to keep moving.&quot;
                      </span>
                    </div>
                  </div>
                </motion.div>
                ...
                <motion.div variants={itemVariants} className="bg-white rounded-xl border border-border p-4 shadow-sm transition-colors hover:border-error-border mt-2">
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      <span className="font-body text-[13.5px] font-semibold text-text-primary block">Duplicate Order</span>
                      <span className="font-data text-[12px] text-error-text flex items-center gap-1 mt-1">
                        <SearchX className="h-3.5 w-3.5" /> Ordered 50 sheets at retail. Yard had 200 sitting untouched.
                      </span>
                    </div>
                  </div>
                </motion.div>
                ...
                <motion.div variants={itemVariants} className="text-center p-3 mt-1">
                  <span className="font-data text-[11px] font-bold uppercase tracking-wider text-error-text bg-error-bg px-3 py-1.5 rounded-full">Blind inventory causes margin bleed</span>
                </motion.div>

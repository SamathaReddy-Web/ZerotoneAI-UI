...
import { X, Mail, SearchX, CalendarOff } from "lucide-react";

const PAIN_POINTS = [
  "Submittals scattered across email: hard to track status",
  "No organized specs or compliance tracking: approval is informal",
  "Lead times not captured: schedule gets surprised by '12 weeks to deliver'",
  "Vendors chasing for approval: confused on status",
];

export function SubmittalsProblemSolution() {
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 lg:gap-24 items-center">
          ...
          <motion.div variants={containerVariants} initial="hidden" animate={isInView ? "visible" : "hidden"} className="flex flex-col max-w-2xl">
            ...
            <motion.h2 variants={itemVariants} className="mb-6 text-balance font-display text-3xl font-bold tracking-tight text-text-primary sm:text-4xl md:text-5xl leading-tight">
              Specs get lost. Lead times get missed. Schedule slips.
            </motion.h2>

            <motion.p variants={itemVariants} className="mb-10 text-lg text-text-secondary leading-relaxed">
              Vendor emails a submittal. You print it, markup by hand, and email back with questions. Vendor sends revision. You review again. Four weeks later the item hasn&apos;t shipped because you never actually approved it: the approval email is three threads deep.
            </motion.p>
            ...
            <div className="rounded-2xl border border-border bg-white p-2 shadow-overlay transform transition-transform hover:-translate-y-1 hover:shadow-2xl duration-500">
              <div className="rounded-xl border border-border-subtle bg-surface/50 overflow-hidden flex flex-col gap-2 p-3">
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div className="w-full">
                      <div className="flex justify-between items-center mb-1">
                        <span className="font-body text-[13px] font-semibold text-text-primary">Re: Submittal Approval?</span>
                        <span className="font-data text-[11px] text-text-muted">Today</span>
                      </div>
                      <span className="font-body text-[12.5px] text-text-secondary line-clamp-2">
                        &quot;Hey, just checking in on those beam specs we sent last month. Need to start fabrication tomorrow if we&apos;re going to hit the delivery date.&quot;
                      </span>
                    </div>
                  </div>
                </motion.div>
                ...
                <motion.div variants={itemVariants} className="bg-white rounded-xl border border-border p-4 shadow-sm transition-colors hover:border-error-border mt-2">
                  <div className="flex items-start gap-3">
                    <CalendarOff className="h-5 w-5 text-error-text mt-0.5" />
                    <div>
                      <span className="font-body text-[13.5px] font-semibold text-text-primary block">Critical Path Delay</span>
                      <span className="font-data text-[12px] text-error-text flex items-center gap-1 mt-1">
                        <SearchX className="h-3.5 w-3.5" /> 12-week lead time missed by 3 weeks
                      </span>
                    </div>
                  </div>
                </motion.div>
                ...
                <motion.div variants={itemVariants} className="text-center p-3 mt-1">
                  <span className="font-data text-[11px] font-bold uppercase tracking-wider text-error-text bg-error-bg px-3 py-1.5 rounded-full">Approvals shouldn&apos;t live in your inbox</span>
                </motion.div>

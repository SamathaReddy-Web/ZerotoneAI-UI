...
export type CardStackItem = {
  ...
  ctaLabel?: string;
  tag?: string;
  // Make it flexible enough for our custom data
  [key: string]: any;
};
...
export type CardStackProps<T extends CardStackItem> = {
  ...
  /** Custom external navigation renderer (optional) */
  renderNavigation?: (state: { active: number; setActive: (idx: number) => void }) => React.ReactNode;
};
...
export function CardStack<T extends CardStackItem>({
  ...
  return (
    ...
    >
      {/* Stage */}
      <div
        className="relative w-full outline-none focus-visible:ring-2 focus-visible:ring-primary-500 rounded-2xl"
            ...
            {items.map((item, i) => {
              ...
              return (
                <motion.div
                  key={item.id}
                  className={cn(
                    "absolute bottom-0 overflow-hidden will-change-transform select-none rounded-[2rem]",
                    isActive
      ...
      {renderNavigation ? renderNavigation({ active, setActive }) : showDots ? (
